#include <iostream>
#include <string>
using namespace std;
int main()
{
    //declare necessary variables
    int age;
    string childname, intrest;
    cout << "please enter child's fullname " <<endl;
    //used to get all the names including the spaces.cin will terminate after a space is inserted
    getline(cin, childname);
    cout << "Please Enter Child's Age" <<endl;
    cin >>age;
    if(age>=4 && age<=10)
    {
        cout <<"please enter the childs intrest "<< endl;
        cin >> intrest;

    
        if(intrest == "soccer") //First nested if
        {
            cout << childname << "has been admited and assigned to the Soccer play group." <<endl;
        }
        if(intrest == "art") //second nested if
        {
            cout << childname << "has been admited and assigned to the Art play group." <<endl;
        }
        else
        {
            cout << childname << "has been admited and assigned to other groups." <<endl;
        }
    }
    else
    {
        cout << "Admission unsuccessful." << childname << " age is invalid";
    }
    return 0;
}
